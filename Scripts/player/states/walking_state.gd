...
func _update(delta: float) -> void:
	...
	if Input.is_action_pressed("ControlSprint"): set_state(player_reference.sprinting)
	if isMovingButNotInputing: set_state(player_reference.moving)
	elif isNotMoving: set_state(player_reference.idle)
		
	if Input.is_action_pressed("LeftShiftCrouch") and player_reference.is_on_floor(): set_state(player_reference.crouching)
